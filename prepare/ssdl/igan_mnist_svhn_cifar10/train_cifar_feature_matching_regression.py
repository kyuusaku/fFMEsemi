...
loss_lab = T.sum(T.sum(T.pow(output_lab - label_matrix, 2), axis=1)) # Squared Error
l_gen = T.sum(T.sum(T.pow(output_gen, 2), axis=1)) # L2 norm
l_unl = T.sum(T.sum(output_unl * T.log(output_unl), axis=1))
