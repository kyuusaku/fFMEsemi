...
%% para
save_path = 'result/mnist8m/semi';
if ~exist(save_path, 'dir')
    mkdir(save_path);
end
record_path = fullfile(save_path, 'record-5');
if ~exist(record_path, 'dir')
    mkdir(record_path);
end

para.dataset = 'MNIST8m';
para.dataPath = fullfile(pwd, 'data');
para.iter = 20;
para.type = 'equal';
para.pca_preserve = 50;
para.p = [10];% number of label per class
para.s = 3; % anchor
para.cn = 10;
...
if ~exist(pca_data, 'file')
    ...
    clear fea gnd split;
    tic;X_pca = X_train(:, randsample(1:numel(Y_train), 100000));
    X_pca = full(X_pca);
    [U, M] = pca(X_pca, para.pca_preserve); clear X_pca;
    fprintf('learn pca done. ');toc;
    tic;X_train_tmp = zeros(para.pca_preserve, size(X_train, 2));
    ...
    clear U M;
    fprintf('preprocess data done. ');toc;
    % save
