function result_efFME = run_efFME_semi_para(...
...
%%
fprintf('******** Runing efFME semi ***********\n');

class = unique(Y_train);

np = numel(label);
result_efFME = cell(np, 1);
for i = 1 : np
    [errs, best_train, best_test, average_time] = ...
        iner_run_efFME_semi(B, X_train, Y_train, X_test, Y_test, class, ...
    ...
    result.p = result.p(1);
    result_efFME{i} = result;
    disp(result);
end    
fprintf('done.\n');
end

function [errs, best_train, best_test, average_time] = iner_run_efFME_semi(B, ...
