function result_eFME = run_eFME_semi_para(...
...
%%
fprintf('******** Runing eFME semi ***********\n');

class = unique(Y_train);

np = numel(label);
result_eFME = cell(np, 1);
for i = 1 : np
    [errs, best_train, best_test, average_time] = ...
        iner_run_eFME_semi(Z, rLz, X_train, Y_train, X_test, Y_test, class, ...
    ...
    result.p = result.p(1);
    result_eFME{i} = result;
    disp(result);
end    
fprintf('done.\n');
end

function [errs, best_train, best_test, average_time] = iner_run_eFME_semi(Z, rLz, ...
