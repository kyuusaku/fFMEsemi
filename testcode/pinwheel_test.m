function pinwheel_test(std1, std2)
%  data test

%% random data
% [fea, gnd] = pinwheel(0.5, 0.6, 2, 5000, 0.25);
% fea = fea';
...
record_path = fullfile(save_path, ['record_2017-pinwheel-' num2str(std1) '-' num2str(std2)]);
...
%% random data
% std = [0.1, 0.3, 0.5]
[fea, gnd] = pinwheel(std1, std2, 2, 5000, 0.25);
fea = fea';
gscatter(fea(1,:)', fea(2,:)', gnd);
