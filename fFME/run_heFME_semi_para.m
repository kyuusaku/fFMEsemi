function result_heFME = run_heFME_semi_para(...
    X_train, Y_train, X_test, Y_test, Z, rLz, label, ul, mu, gamma)
...
%%
fprintf('******** Runing heFME semi ***********\n');

class = unique(Y_train);

np = numel(label);
result_heFME = cell(np, 1);
for i = 1 : np
    [errs, best_train, best_test, average_time] = ...
        iner_run_heFME_semi(Z, rLz, X_train, Y_train, X_test, Y_test, class, ...
    ...
    result.p = result.p(1);
    result_heFME{i} = result;
    disp(result);
end    
fprintf('done.\n');
end

function [errs, best_train, best_test, average_time] = iner_run_heFME_semi(Z, rLz, ...
...
for pmu = 1 : n_mu
    for pgamma = 1 : n_gamma
        ...
        for t = 1 : iter
            ...
            [W, b, F_train] = eFME_semi(X_train, Z, rLz, Y, p);
