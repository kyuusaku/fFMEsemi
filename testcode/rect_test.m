% pinwheel+noise data test

%% random data
[fea, gnd] = pinwheel(0.3, 0.3, 2, 4000, 0.25);
fea = fea';

noise = 4.*rand(2, 2000) - 2;
noise_gnd = ones(2000, 1);
noise_gnd(noise(1,:)<0) = 2;

fea = [fea noise];
gnd = [gnd; noise_gnd];

tmp = randperm(numel(gnd));
fea = fea(:,tmp);
gnd = gnd(tmp);
...
record_path = fullfile(save_path, 'record_20150901-pinnoise');
